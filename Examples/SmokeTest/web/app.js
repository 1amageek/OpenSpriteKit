// OpenSpriteKit smoke-test loader.
// The build script stages runtime.mjs from the resolved JavaScriptKit checkout
// so the JavaScript and WebAssembly ABI versions remain identical.
...
function showError(msg) {
    ...
    console.error("[OSKSmoke]", msg);
}

async function main() {
    try {
        setStatus("Loading WASM module...");
        const response = await fetch("OSKSmoke.wasm");
        if (!response.ok) {
            throw new Error("Failed to load OSKSmoke.wasm: " + response.status);
        }
