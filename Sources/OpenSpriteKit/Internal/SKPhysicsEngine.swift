...
internal final class SKPhysicsEngine {
    ...
    private func getAABB(for body: SKPhysicsBody) -> CGRect? {
        ...
        return worldAABB(for: body.shape, body: body, node: node)
    }

    private func worldAABB(for shape: SKPhysicsBodyShape, body: SKPhysicsBody, node: SKNode) -> CGRect? {
        guard let localRect = localBoundingRect(for: shape, body: body) else { return nil }
